...
import com.bilgiyazan.malzemeiste.ModelAdapter.MoreAdapterModelYedek;
import com.bilgiyazan.malzemeiste.ModelAdapter.MoreDetailsAdapterModelYedek;
import com.bilgiyazan.malzemeiste.R;
import com.bilgiyazan.malzemeiste.ViewHolder.MoreViewHolder_Yedek;
import com.bilgiyazan.malzemeiste.ViewHolder.YedekViewHeaderHolder;
import com.bilgiyazan.malzemeiste.ViewHolder.YedekViewTitleHolder;
 ...
 */
public class YazilimAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    Activity activity;
    List<CorelDraw> corelDrawList;

    private int lastPosition = -1;
    private int mHeaderDisplay;
    private boolean mMarginsFixed;

    public YazilimAdapter(Context context, int headerMode, Activity activity, Yazilim yazilim) {

        corelDrawList = new ArrayList<>();
        corelDrawList = yazilim.getCorelDrawList();

        mContext = context;
        this.activity = activity;
        yazilim_array = context.getResources().getStringArray(R.array.yazilim);
        ...
        for (int i = 0; i < yazilim_array.length; i++) {
            String header = yazilim_array[i].substring(0, 1);
            if (!TextUtils.equals(lastHeader, header)) {
                ...
                lastHeader = header;
                headerCount += 1;
                mItems.add(new YazilimAdapter.LineItem(header, true, false, false, false, sectionManager, sectionFirstPosition));
            }

            mItems.add(new YazilimAdapter.LineItem(yazilim_array[i], false, false, false, true, sectionManager, sectionFirstPosition));

            mItems.add(new YazilimAdapter.LineItem(items_name[0], false, false, true, false, sectionManager, sectionFirstPosition));
               /* for (int j = 0; j < sub_items_name.length; j++) {
                    mItems.add(new YazilimAdapter.LineItem(sub_items_name[j], false, true, false, false, sectionManager, sectionFirstPosition));
                }*/
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        if (getItemViewType(position) == VIEW_TYPE_HEADER) {
            ...
            final YazilimAdapter.LineItem item = mItems.get(position);
...
*/
            ...
            // Overrides xml attrs, could use different layouts too.
            final YazilimAdapter.LineItem item = mItems.get(position);
            if (mItems.get(position - 1).text.equals("COREL DRAW")) {
                List<CorelDraw> mContentItems = new ArrayList<>();
                if (!corelDrawList.isEmpty()) {

                    if (corelDrawList.size() >= 4) {
                        for (int i = 0; i < 4; i++) {

                                mContentItems.add(new CorelDraw(corelDrawList.get(i).getModel(), corelDrawList.get(i).getKod(), corelDrawList.get(i).getFiyat(), corelDrawList.get(i).getURL(), corelDrawList.get(i).getDescription(), corelDrawList.get(i).getRate(), corelDrawList.get(i).getShare_Link()));
                    ...
                    NormalItemAdapter_Yazilim mAdapter;
                    mAdapter = new NormalItemAdapter_Yazilim(mContentItems, activity);
    ...
    private void notifyHeaderChanges() {
        for (int i = 0; i < mItems.size(); i++) {
            YazilimAdapter.LineItem item = mItems.get(i);
            if (item.isHeader) {
                notifyItemChanged(i);
            }
        }
